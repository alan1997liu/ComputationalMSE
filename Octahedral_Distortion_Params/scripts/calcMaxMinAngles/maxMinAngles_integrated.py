...
for cifFile in os.listdir():
    
    # Check only reading in vasp files in current directory.
    if (".vasp" not in cifFile):
            continue
    
    # Read in structure.
    struct= vasp.inputs.Poscar.from_file(filename).structure
    
    # Now, have to define here what the cation and anion is.
    B_atom = "Pb"
    X_atom = "I"
    B_coordination = 6
    ...
    B_indexes, X_indexes, B_coords, X_coords = \
            getSpeciesInfo(supercell_species_arr, B_atom, X_atom)
    octahedral_array = get_octahedrals(B_indexes, X_indexes, B_coordination)
    center_octahedrals = \
            get_center_octahedrals(octahedral_array, numUnitCells)
